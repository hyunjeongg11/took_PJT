  ...
  return (
    <div className="flex flex-col items-center p-5 h-screen font-nanum">
      <div className="w-full flex items-center justify-center mb-5 border-b border-gray-300 pb-2 relative">
        <AiOutlineLeft
          className="absolute left-3 text-2xl cursor-pointer"
            ...
            <div className="flex items-center ml-2 flex-1">
              ...
              <div className="flex flex-col">
                <div className="flex items-center text-lg font-bold">
                  {account.bank_name}
                  {index === 0 && (
                    <span className="bg-orange-100 text-orange-600 text-xs ml-2 px-2 py-1 rounded-full">
                      주계좌
                    </span>
                  )}
                </div>
      ...
      <button className="w-[calc(100%-40px)] py-3 rounded-full border-none text-white text-lg font-bold cursor-pointer bg-orange-500 mt-5 absolute bottom-5 left-1/2 transform -translate-x-1/2">
        + 결제 수단 추가
      </button>
    </div>
  );
