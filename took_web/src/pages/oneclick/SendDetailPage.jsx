  ...
  return (
    <div className="flex flex-col items-center bg-white font-[Nanum_Gothic] h-screen">
      ...
      <div className="flex flex-col items-center mb-4">
        <img src={getProfileImagePath(1)} alt="profile" className="w-16 h-16 mb-2 mt-5" />
        <div className="text-sm mb-3 ">{maskName(tempMember.userName)}</div>
        <div className="text-lg mb-1">{maskName(tempMember.userName)} 님에게 보낼 금액</div>
        <div className="text-3xl font-bold text-black mb-2">{formatNumber(tempMember.cost)}원</div>
        <div className="w-64 border-gray-300 py-3 mt-5">
          <div className="flex justify-between w-full mt-1 mb-2 px-1">
            ...
            <div className="text-sm">{formatNumber(tempMember.totalCost)} / <span className="font-bold">{formatNumber(tempMember.cost)}원</span></div>
          </div>
          {tempMember.status ? (
            <>
              <div className="flex justify-between w-full mt-1 mb-1 px-1">
                <div className="text-sm">송금 상태</div>
                <div className="text-sm font-bold">{tempMember.status ? '송금 완료' : '송금 미완료'}</div>
              </div>
              <div className="w-64 border-t border-b border-gray-300 py-3">
                <div className="flex justify-between w-full mt-1 mb-2 px-1">
                  <div className="text-sm">출금 계좌</div>
                  <div className="text-sm font-bold">{tempMember.bank}({tempMember.accountNumber.slice(-4)})</div>
                </div>
              </div>
            </>
          ) : ('')}
            <div className="flex justify-between w-full mt-1 mb-1 px-1">
                ...
                <div className="text-sm font-bold">{formatDate(tempMember.createdAt)}</div>
            </div>
        </div>
        {tempMember.status ? (
            <button
