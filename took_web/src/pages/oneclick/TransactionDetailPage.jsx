...
const TransactionDetailPage = () => {
  ...
  const { userName, imgNo, cost, type, bankName, accountNum, createdAt } = transaction;
  ...
  return (
    <div className="flex flex-col bg-white max-w-[360px] mx-auto relative h-screen">
      ...
      <div className="flex flex-col items-center mt-8">
        <img src={getProfileImagePath(imgNo)} alt={userName} className="w-16 h-16 mb-4" />
      ...
      <div className="mt-8 px-4">
        <div className="border-t border-b border-gray-300 py-4 ml-2 mr-2">
          <div className="flex justify-between py-2">
            ...
            <span className="font-bold">{bankName}({accountNum.slice(-4)})</span>
          </div>
          <div className="flex justify-between py-2">
            <span className="text-gray-500">일시</span>
            <span className="font-bold">{new Date(createdAt).toLocaleString('ko-KR', { year: 'numeric', month: 'long', day: 'numeric', hour: 'numeric', minute: 'numeric' })}</span>
          </div>
        </div>
      </div>
    </div>
