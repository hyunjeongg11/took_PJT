...
const tempMember = [
  ...
  { member_seq: 2, party_seq: 1, user_seq: 2, userName: '정희수', imgNo: 23, cost: 7300, real_cost: 7500, status: false, receive: false, is_leader: false, created_at: '2024-07-06T00:23:00' },
  { member_seq: 3, party_seq: 1, user_seq: 3, userName: '차민주', imgNo: 18, cost: 13800, real_cost: 14500, status: true, receive: false, is_leader: false, created_at: '2024-07-06T00:23:00' },
];
      ...
      <div className="flex flex-col mt-4 px-4 font-[Nanum_Gothic] max-h-[calc(100%-160px)] relative">
        <div className="p-5 rounded-xl shadow-lg border border-inherit h-full overflow-y-scroll pb-6">
          <div className="flex items-center justify-between mb-4">
            <div className="text-gray-500 text-sm">{formatDate(tempTaxi.created_at)}</div>
          </div>
          <div className="flex items-center mb-6">
            ...
            <div className="ml-4 font-bold relative flex-grow">
              ...
              <div className="text-lg flex items-center">{formatNumber(tempTaxi.cost)} 원</div>
            </div>
            <img src={allMembersCompleted ? complete : incomplete} alt="status" className="w-18 h-16" />
          </div>

          {tempMember.map((member, index) => {
            const balance = member.cost - member.real_cost;
            return (
              <div key={member.member_seq} className={`mb-4 ${index === tempMember.length - 1 ? '' : 'border-b border-gray-300'}`}>
                <div className="flex items-center mb-3 justify-between">
                  <div className="flex items-center">
                    <img src={getProfileImagePath(member.imgNo)} alt={member.userName} className="w-9 h-9 mr-4" />
                    <div className="font-bold flex items-center">
                      ...
                      {!member.is_leader && !member.status && (
                        <FaBell className="ml-2 text-yellow-400 cursor-pointer" onClick={() => setPopupUserName(member.userName)} />
                      )}
                    </div>
                  </div>
                  <div className="text-right font-bold">
                    <span className="text-sm">{formatNumber(member.cost)} 원</span>
                    <div className={`text-sm ${member.status ? 'text-black' : 'text-red-500'}`}>
