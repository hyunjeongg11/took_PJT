...
function PaymentInputPage() {
    ...
    const addNewPayment = () => {
        ...
        const newPayments = { ...payments, [Object.keys(payments).length + 1]: newPayment };
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-screen">
            ...
            >
                {Object.values(payments).map((payment, index) => (
                    <SwiperSlide key={index}>
                        <PaymentCard payment={payment} setPayment={(updatedPayment) => {
                            const updatedPayments = { ...payments, [index + 1]: updatedPayment };
                            setPayments(updatedPayments);
                        }} onDelete={(userIndex) => handleDeleteUser(index + 1, userIndex)} onCardDelete={() => handleDeleteCard(index + 1)} />
                    </SwiperSlide>
                ))}
            </Swiper>
            ...
            <Link to="/dutch/total">
            <button onClick={() => { console.log(payments) }} className="bg-main px-12 py-2 shadow font-bold text-white rounded-full">
                정산 요청하기
            </button>
            </Link>
        </div>
