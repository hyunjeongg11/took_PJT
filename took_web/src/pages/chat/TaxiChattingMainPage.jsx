...
import { FaPaperPlane, FaArrowDown, FaTimes, FaBars, FaSignOutAlt } from 'react-icons/fa';
import { IoIosSettings } from "react-icons/io";
import { FaLocationDot, FaCrown, FaCalculator } from "react-icons/fa6";
import { MdAdd } from 'react-icons/md';

const tempMember = [
  { member_seq: 1, party_seq: 1, user_seq: 1, userName: '차민주', imgNo: 19, cost: 13000, real_cost: 12000, status: true, receive: false, is_leader: true, gender: false, created_at: '2024-07-06T00:23:00', destination: '부산시 강서구 녹산산단335로 7 송정삼정그린코아더시티' },
  { member_seq: 2, party_seq: 1, user_seq: 2, userName: '조현정', imgNo: 20, cost: 8000, real_cost: 7500, status: true, receive: false, is_leader: false, gender: false, created_at: '2024-07-06T00:23:00', destination: '부산시 강서구 명지국제5로 170-5 명일초등학교' },
  { member_seq: 3, party_seq: 1, user_seq: 3, userName: '정희수', imgNo: 18, cost: 16000, real_cost: 14500, status: true, receive: false, is_leader: false, gender: false, created_at: '2024-07-06T00:23:00', destination: '부산시 서구 대영로73번길 39' },
];
...
function TaxiChattingMainPage() {
  ...
  return (
    <div className="flex flex-col bg-[#FFF7ED] max-w-[360px] mx-auto relative h-screen">
      ...
      {showMenu && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-end z-50">
          <div className="w-4/5 h-full bg-white shadow-md p-4 relative">
            <button onClick={handleMenuToggle} className="text-gray-400 focus:outline-none absolute top-5 right-4">
              <FaTimes className="w-5 h-5" />
            </button>
            <div className="text-base font-bold mt-6 ml-1 mb-4">경로</div>
            <ul>
              {tempMember.map((member) => (
                <li key={member.user_seq} className="flex items-center justify-between mb-2 py-1">
            ...
            <ul>
              {tempMember.map((member) => (
                ...
                >
                  ...
                  {tempTaxi.user_seq === tempTaxi.payer && member.user_seq === tempTaxi.payer && (
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center text-center text-base items-center z-50">
          <div className="bg-white rounded-2xl p-6 w-64">
            ...
            <div className="flex justify-between">
              <button onClick={closeModal} className="bg-gray-200 text-black text-sm px-8 py-2 rounded-lg font-bold">취소</button>
              {modalMessage === '채팅방을 나가시겠습니까?' ? (
                <button onClick={handleConfirmLeaveChatting} className="bg-main text-white text-sm px-8 py-2 rounded-lg font-bold">확인</button>
              ) : modalMessage === '탑승 처리하시겠습니까?' ? (
                <button onClick={handleConfirmBoardTaxi} className="bg-main text-white text-sm px-8 py-2 rounded-lg font-bold">확인</button>
              ) : (
                <button onClick={handleConfirmDoneTaxi} className="bg-main text-white text-sm px-8 py-2 rounded-lg font-bold">확인</button>
              )}
            </div>
          </div>
        </div>
      ...
      <div className="flex-grow overflow-y-scroll px-4 py-2 space-y-4 relative" onScroll={handleScroll} ref={scrollContainerRef} style={{ paddingBottom: '100px' }}>
      ...
      <div className={`absolute bottom-0 left-0 w-full ${isKeyboardVisible ? 'hidden' : ''}`}>
        ...
        <div className="w-full px-2 py-2 bg-white flex items-center relative">
          ...
          {showScrollButton && (
            <button onClick={scrollToBottom} className="absolute bottom-16 right-4 p-2 bg-white text-gray-500 rounded-full shadow-md">
      ...
      {showActionIcons && (
        <div className="w-full px-4 py-2 bg-white flex justify-around absolute bottom-14 left-0 z-50" ref={actionIconsRef}>
