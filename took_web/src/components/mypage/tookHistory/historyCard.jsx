    ...
    <div className={`flex flex-col font-bold bg-[#FBFBFB] border border-slate-100 p-4 mb-4 rounded-2xl shadow-lg ${status === "정산완료" ? "opacity-50" : ""}`}>
      <div className="flex justify-between items-center mb-3 text-sm text-gray-500">
        <div className="flex items-center">
          ...
          <span>|</span>
          <span className={`ml-4 ${status === "정산완료" ? "text-gray-500" : "text-blue-500"}`}>{status}</span>
        </div>
        <div>
          {status === "정산 진행 중" ? ( 
            <button className="text-white text-xs font-bold bg-neutral-400/75 px-4 py-1.5 shadow-md rounded-full">채팅</button>
          ) : (
      ...
      <div className="flex items-center">
        <img src={categoryIcons[category]} alt={category} className="w-14 h-14 mr-2" />
