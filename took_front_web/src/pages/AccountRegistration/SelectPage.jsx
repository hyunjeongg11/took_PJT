...
function SelectPage() {
  ...
  const handleSelect = (name) => {
    let fullName = name;
    if (selectedTab === "은행" && !name.endsWith("뱅크") && !name.endsWith("우체국") && !name.endsWith("금고")) {
      fullName = `${name}은행`;
    }
  ...
  return (
    <div className="flex flex-col items-center p-5 relative h-screen font-[Nanum_Gothic]">
      <div className="w-full flex items-center justify-between mb-2 border-b border-gray-300 pb-2">
        ...
        <AiOutlineClose className="text-2xl cursor-pointer flex-shrink-0" onClick={() => navigate("/account")} />
      </div>
      ...
      <div className="grid grid-cols-4 gap-2">
        {selectedTab === "은행" && banks.map((bank) => (
          <div key={bank.name} className="flex flex-col items-center cursor-pointer" onClick={() => handleSelect(bank.name)}>
        ...
        {selectedTab === "증권사" && stocks.map((stock) => (
          <div key={stock.name} className="flex flex-col items-center cursor-pointer" onClick={() => handleSelect(stock.name)}>
            <img src={stock.icon} alt={stock.name} className="w-12 h-12 mb-1" />
